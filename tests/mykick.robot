...
*** Test Cases ***
Check if mykick is installed correctly
...
Check if mykick can be configured
    ${rc} =    Execute Command    api-cli run module/${module_id}/configure-module --data '{}'
    ...    return_rc=True  return_stdout=False
    Should Be Equal As Integers    ${rc}  0

Check if mykick works as expected
    ${rc} =    Execute Command    curl -f http://127.0.0.1/mykick/
    ...    return_rc=True  return_stdout=False
    Should Be Equal As Integers    ${rc}  0

Check if mykick is removed correctly
